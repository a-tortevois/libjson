...
json_object *json_parse(char *str, size_t str_len, json_object mem_stack[], int mem_stack_size) {
    if (str != NULL && str_len > 0 && mem_stack != NULL && mem_stack_size > 0) {
    ...
    for (; parser.pos < str_len && str[parser.pos] != '\0'; parser.pos++) {
        ...
        switch (c) {
            ...
            case ']': {
                ...
                if (parser.current->type == JSON_UNDEFINED /* && (parser.current->key == NULL || parser.current->value == NULL) */) {
                    json_free_child();
                }
...
int json_getInteger(json_object *json_obj, int *result) {
    if (json_obj && json_obj->type == JSON_INTEGER) {
        return parse_int(json_obj->value, result);
    }
    return -1;
}
...
int json_getArrayLength(json_object *json_obj) {
...
json_object *json_getArrayIndex(json_object *json_obj, int index) {
    json_object *this = NULL;
...
json_object *json_getFirst(json_object *json_obj) {
    if (json_obj && (json_obj->type == JSON_OBJECT || json_obj->type == JSON_ARRAY)) {
