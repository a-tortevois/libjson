...
static void test_12(void) {
    ...
    strcpy(buffer, "{\"key\":{\"key_1\":\"value_1\",\"key_2\":\"value_2\"}}");
    printf("--- Run %s ---\n", __FUNCTION__);
    printf(" Parse: %s\n\n", buffer);
    ...
    assert("[1].type == JSON_OBJECT: ", json_data_stack[1].type == JSON_OBJECT);
    ...
    assert("[2].key == \"key_1\": ", strcmp(json_data_stack[2].key, "key_1") == 0);
    ...
    assert("[3].key == \"key_2\": ", strcmp(json_data_stack[3].key, "key_2") == 0);
