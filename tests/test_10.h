...
static void test_10(void) {
    ...
    strcpy(buffer, "{\"key_1\":\"value_1\",\"key_2\":\"value_2\"}");
    printf("--- Run %s ---\n", __FUNCTION__);
    printf(" Parse: %s\n\n", buffer);
    ...
    assert("[0].size == 2: ", json_data_stack[0].size == 2);

    assert("[1].key == \"key_1\": ", strcmp(json_data_stack[1].key, "key_1") == 0);
    ...
    assert("[1].value == \"value_1\": ", strcmp(json_data_stack[1].value, "value_1") == 0);

    assert("[2].key == \"key_2\": ", strcmp(json_data_stack[2].key, "key_2") == 0);
    assert("[2].type == JSON_STRING: ", json_data_stack[2].type == JSON_STRING);
    assert("[2].parent == &[0]: ", json_data_stack[2].parent == &json_data_stack[0]);
    assert("[2].next == NULL: ", json_data_stack[2].next == NULL);
    assert("[2].value == \"value_2\": ", strcmp(json_data_stack[2].value, "value_2") == 0);

    printf("\n Result:\n");
    json_object *obj = json_getFirst(parsed);
    while (obj != NULL) {
        printf(" %s -> %s\n", obj->key, obj->value);
    ...
    do {
        printf(" %s -> %s\n", obj->key, obj->value);
    } while ((obj = json_getNext(obj)) != NULL);
    */
